<script setup lang="ts">
import { useSeoMeta, useHead } from "@vueuse/head";
import { ref } from "vue";

const fullName = "Nexus F. Paloma";
const title = `${fullName} | Personal Page`;
const description = "Resume-style personal page of Nexus F. Paloma.";
const profilePhoto = "/npaloma-photo.jpg";
const fallbackPhoto = "/PALOMA.jpg";
const currentPhoto = ref(profilePhoto);

function onPhotoError() {
  currentPhoto.value = fallbackPhoto;
}

useSeoMeta({
  title: () => title,
  description: () => description,
  charset: "utf-8",
  viewport: "width=device-width, initial-scale=1.0",
});

useHead({
  link: [
    { rel: "icon", type: "image/png", href: "/logo.png" },
    { rel: "stylesheet", href: "/reset.css" },
    { rel: "stylesheet", href: "/custom.css" },
    { rel: "preconnect", href: "https://fonts.googleapis.com" },
    { rel: "preconnect", href: "https://fonts.gstatic.com", crossorigin: "" },
    {
      rel: "stylesheet",
      href: "https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;700&family=Source+Serif+4:opsz,wght@8..60,400;8..60,700&display=swap",
    },
  ],
});
</script>

<template>
  <client-only>
    <NavBar navAdminMode="" />

    <main class="np-wrap">
      <section class="np-hero">
        <img
          class="np-avatar"
          :src="currentPhoto"
          @error="onPhotoError"
          alt="Nexus F. Paloma professional profile"
        />

        <div class="np-intro">
          <p class="np-kicker">Personal Page</p>
          <h1 class="np-name">NEXUS F. PALOMA</h1>
          <p class="np-contact">Leyte, PH | +63 927 197 0911 | nexuspaloma24@gmail.com</p>
          <p class="np-contact">github.com/NexusfPaloma | linkedin.com/in/palomaNexus</p>
          <p class="np-summary">Aspiring Full-Stack Developer/Game Developer</p>
        </div>
      </section>

      <section class="np-grid">
        <article class="np-card np-card-wide">
          <h2>Education</h2>
          <h3>Visayas State University</h3>
          <p class="np-muted">Bachelor of Science in Computer Science</p>
          <p class="np-muted">2024 - Present</p>
          <ul>
            <li>Ongoing bachelor&apos;s degree (2nd Year).</li>
          </ul>
        </article>

        <article class="np-card np-card-wide">
          <h2>Experience</h2>
          <h3>Front End Website</h3>
          <p class="np-muted">Dec 2025 - Present</p>
          <ul>
            <li>Developed 1 responsive, mobile-first web interfaces using Figma and modern CSS frameworks.</li>
            <li>Optimized asset loading handling over 200 lines of code.</li>
            <li>General UX graphic designing with custom and free-use image/art materials.</li>
            <li>Refactored legacy code to adhere to web standards, ensuring WCAG 2.1 compliance.</li>
          </ul>

          <h3>Back End Server-Side Logic and Database</h3>
          <p class="np-muted">Aug 2024 - Jun 2025</p>
          <ul>
            <li>Designed normalized SQL schemas and indexed tables to improve data retrieval.</li>
            <li>Implemented data handling for 50+ mock users during testing.</li>
            <li>Provided CLI front-end tooling for database updates through phpMyAdmin.</li>
          </ul>

          <h3>Server Hosting (Video Games)</h3>
          <p class="np-muted">Dec 2025 - Jan 2026</p>
          <ul>
            <li>Managed 100+ mod dependencies and resolved conflicts to avoid runtime crashes.</li>
            <li>Maintained a Windows-based Minecraft server with 99% uptime for active players.</li>
          </ul>
        </article>

        <article class="np-card">
          <h2>Skills</h2>
          <p><strong>Programming Languages:</strong> Java, C/C++, Python, JavaScript</p>
          <p><strong>Web Technologies:</strong> HTML, CSS, Figma</p>
          <p><strong>Database and Server:</strong> SQL(MySQL), phpMyAdmin, Database Design, Linux/Windows Server Administration</p>
          <p><strong>Tools and Version Control:</strong> Git, GitHub, GitKraken, VSCode, Photoshop</p>
        </article>

        <article class="np-card">
          <h2>Projects</h2>
          <ul>
            <li><strong>itch.io Game Jam Entry:</strong> Volunteered and coordinated in a 2-man team developing a 2D rhythmic button-masher game within 1 week using Godot.</li>
            <li><strong>Student Database SQL:</strong> Built a CRUD app with phpMyAdmin, executed 50+ test operations, and implemented 15+ SQL triggers and stored procedures for validation and reporting.</li>
          </ul>
        </article>
      </section>
    </main>

    <Copyright />
  </client-only>
</template>

<style scoped>
.np-wrap {
  min-height: calc(100vh - 120px);
  padding: 40px 20px 60px;
  background:
    radial-gradient(circle at 20% 15%, #f6e5d6 0%, transparent 40%),
    radial-gradient(circle at 80% 10%, #dceaf6 0%, transparent 35%),
    linear-gradient(135deg, #fcfaf6 0%, #f2f5f8 100%);
}

.np-hero {
  max-width: 980px;
  margin: 0 auto;
  display: grid;
  grid-template-columns: 140px 1fr;
  gap: 24px;
  align-items: center;
  background: rgba(255, 255, 255, 0.82);
  border: 1px solid #d2d9e0;
  border-radius: 18px;
  padding: 24px;
  box-shadow: 0 12px 24px rgba(24, 34, 45, 0.08);
}

.np-name {
  font-size: clamp(1.9rem, 3vw, 2.8rem);
  line-height: 1.1;
  margin: 8px 0;
}

.np-avatar {
  width: 132px;
  height: 132px;
  border-radius: 50%;
  border: 3px solid #223347;
  background: #fff;
  object-fit: cover;
  object-position: center 20%;
  display: block;
  flex-shrink: 0;
}

.np-intro {
  font-family: "Space Grotesk", sans-serif;
  color: #1a2a3a;
}

.np-kicker {
  font-size: 0.9rem;
  text-transform: uppercase;
  letter-spacing: 0.12em;
  color: #4a6075;
  margin: 0;
}

.np-contact {
  margin: 4px 0;
  color: #2b455f;
  font-weight: 500;
}

.np-summary {
  margin-top: 10px;
  max-width: 72ch;
  line-height: 1.6;
}

.np-grid {
  max-width: 980px;
  margin: 22px auto 0;
  display: grid;
  grid-template-columns: repeat(2, minmax(0, 1fr));
  gap: 16px;
}

.np-card {
  background: rgba(255, 255, 255, 0.84);
  border: 1px solid #d2d9e0;
  border-radius: 14px;
  padding: 18px 18px 16px;
  box-shadow: 0 10px 20px rgba(24, 34, 45, 0.06);
  color: #1a2a3a;
  font-family: "Source Serif 4", serif;
}

.np-card-wide {
  grid-column: 1 / -1;
}

.np-card h2 {
  margin: 0 0 10px;
  font-family: "Space Grotesk", sans-serif;
  font-size: 1.05rem;
  color: #1e3550;
}

.np-card h3 {
  margin: 12px 0 4px;
  font-size: 1.08rem;
}

.np-card p {
  margin: 4px 0;
  line-height: 1.55;
}

.np-muted {
  color: #4c6074;
}

.np-card ul {
  margin: 6px 0 0;
  padding-left: 18px;
}

.np-card li {
  margin: 4px 0;
  line-height: 1.45;
}

@media (max-width: 760px) {
  .np-hero {
    grid-template-columns: 1fr;
    text-align: center;
    justify-items: center;
  }

  .np-summary {
    max-width: 100%;
  }

  .np-grid {
    grid-template-columns: 1fr;
  }

  .np-card-wide {
    grid-column: auto;
  }
}
</style>
